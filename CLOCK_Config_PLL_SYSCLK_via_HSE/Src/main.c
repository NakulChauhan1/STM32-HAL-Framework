 ...
 *  84 MHz
 *  120 MHz
 *  Also write code to run mcu at max frequency ie 180Mhz in our case
 */
...
int main()
{
	...
	uint8_t clk = SYS_CLK_FREQ_50MHz;						/*!< clk is value of system clock
...
void SystemClockConfig( uint8_t clk )
{
	...
	osc_init.HSEState = RCC_HSE_BYPASS;						//bypass because we do not have oscillator between the pins, so we have to take frequency from external source (can be timer, RTC etc)
	//osc_init.HSICalibrationValue = 16;         			//not required in HSE because its already stable
	...
	if ( HAL_RCC_OscConfig ( &osc_init ) != HAL_OK )				//it just activates the selected oscillator and waits till its not on
	{
		Error_Handler();
	}
