...
void HAL_TIM_OC_MspInit(TIM_HandleTypeDef *htim)
{
	...
	HAL_NVIC_SetPriority(TIM2_IRQn, 15, 0);
	 ...
	 //3. Configure gpios to behave as timer2 channel 1,2,3 and 4
	...
	/*Pin Muxing:
			as we are using using pins to produce outputs to timer, therefore we have to give alternate functionality to those gpios
		  	  ie configure gpio behave as TIMER2 channel 1, 2 etc,
	...
	//Configuring PA0, PA1 as Output Channel
	GPIO_Init.Pin = GPIO_PIN_0 | GPIO_PIN_1;
	GPIO_Init.Alternate = GPIO_AF1_TIM2;
	HAL_GPIO_Init ( GPIOA, &GPIO_Init );

	//Configuring PB10, PB2 as Output Channel
	GPIO_Init.Pin = GPIO_PIN_2 | GPIO_PIN_10;
	GPIO_Init.Alternate = GPIO_AF1_TIM2;
